...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		//echo $this->config->get('config_limit_admin');
		...
		foreach ($results as $result) {
			...
			$data['orders'][] = array(
				...
				'restore'       => $this->url->link('sale/order/restore', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $result['order_id'] . $url, true),
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['shipping_method'] = $order_info['shipping_method'];
			$data['payment_method'] = ($order_info['order_status_id'] != 17) ? $order_info['payment_method'] : 'vor Ort';
              ...
              if ($order_info) {
				...
				foreach ($products as $product) {
					...
					if ($product_info) {
						...
						// print_r($weight_info); die();
						$unit = $weight_info ? $weight_info['unit'] : '';
						// xxxxxx				
