...
class ModelExtensionReportCustomer extends Model {
	...
	public function getOrders_adjust($data = array()) {
		$filter_gram_from = (!empty($data['filter_gram_from'])) ? $data['filter_gram_from'] : 0;
		$filter_gram_to = (!empty($data['filter_gram_to'])) ? $data['filter_gram_to'] : 20000000;
		$filter_price_from = (!empty($data['filter_price_from'])) ? $data['filter_price_from'] : 0;
		$filter_price_to = (!empty($data['filter_price_to'])) ? $data['filter_price_to'] : 0;
		...
		if(empty($data['filter_gram_to']) && empty($data['filter_price_to']))
		  $sql = "select * from (SELECT t.customer_id, t.customer, t.email, t.customer_group, t.status, COUNT(DISTINCT t.order_id) AS orders, SUM(t.products) AS products, SUM(t.total) AS total FROM (" . $sql . ") AS t GROUP BY t.customer_id ORDER BY total DESC) as t where products >= ".$filter_gram_from." AND total >= ".$filter_price_from."";
		else if(empty($data['filter_gram_to']))
		  $sql = "select * from (SELECT t.customer_id, t.customer, t.email, t.customer_group, t.status, COUNT(DISTINCT t.order_id) AS orders, SUM(t.products) AS products, SUM(t.total) AS total FROM (" . $sql . ") AS t GROUP BY t.customer_id ORDER BY total DESC) as t where products >= ".$filter_gram_from." AND total >= ".$filter_price_from." AND total <= ".$filter_price_to."";
		else if(empty($data['filter_price_to']))
		  $sql = "select * from (SELECT t.customer_id, t.customer, t.email, t.customer_group, t.status, COUNT(DISTINCT t.order_id) AS orders, SUM(t.products) AS products, SUM(t.total) AS total FROM (" . $sql . ") AS t GROUP BY t.customer_id ORDER BY total DESC) as t where products >= ".$filter_gram_from." AND total >= ".$filter_price_from." AND products <= ".$filter_gram_to."";
		else 
		  $sql = "select * from (SELECT t.customer_id, t.customer, t.email, t.customer_group, t.status, COUNT(DISTINCT t.order_id) AS orders, SUM(t.products) AS products, SUM(t.total) AS total FROM (" . $sql . ") AS t GROUP BY t.customer_id ORDER BY total DESC) as t where products >= ".$filter_gram_from." AND total >= ".$filter_price_from." AND products <= ".$filter_gram_to." AND total <= ".$filter_price_to."";
