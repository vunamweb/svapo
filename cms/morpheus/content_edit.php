...
elseif ($save) {
	...
	if ($db != "morp_cms_news" && $db != "productkat") 	
		$set = "SET ".$cont."='".addslashes($text)."', edit=1, layout='$layout' ";
	else
		$set = "SET ".$cont."='".addslashes($text)."', edit=1 ";
...
# # $b_chk enthaelt den namen des brick = name der php-vorlage aus dem ordner bricks
				...
				elseif (preg_match("/formular anmeldung/", $b_chk))						$insert_event = 1;
				elseif (preg_match("/formular/", $b_chk)) 								$insert_form = 1;
				elseif (preg_match("/kontakt button team/", $b_chk))					$insert_team_kontakt = 1;
				
				elseif (preg_match("/umbruch/", $b_chk) || preg_match("/sitemap/", $b_chk) ||
					// preg_match("/^kunden/", $b_chk) || 
					preg_match("/^spalte/", $b_chk) ||
...
#				elseif (preg_match("/menu/", $b_chk)) 									$insert_menu = 1;
				...
				elseif (preg_match("/veranstaltung/", $b_chk)) 							$insert_event = 1;
	...
	<tr>";
		...
		<tr>';
				...
				elseif ($insert_link) {
					...
					if(preg_match("/intern/", $brick[0])) {
						$get_navid = getAllInternalID($href);
						$select_ilink = '
						<input type="hidden" name="brick#' .$brick[0] .'#' .$counter .'_a" style="width:200px" value="'.$href.'"  class="getVal yyy">
						<select name="brick#'.$brick[0] .'#'.$counter.'_a" class="form-control select" style="width:150px;">'.$get_navid.'</select>';						
					} else {						
						$select_ilink = '<input type="text" name="brick#' .$brick[0] .'#' .$counter .'_a" style="width:200px" value="'.$href.'"  class="getVal yyy">';						
					}
					
						
					if (preg_match("/TOP/", $brick[0])) $textbox .= "<input type=\"Hidden\" name=\"brick#" .$brick[0] ."#" .$counter ."\" value=\"anker\"  class=\"getVal xxx\"><input type='text' class=\"form-control\" name=\"brick#" .$brick[0] ."#" .$counter ."\" style=\"width:50px\" value=\"$href\">\n";
				...
				elseif ($insert_form || $insert_templ || $insert_termin || $insert_news || $insert_loesung || $galerie || $insert_branche || $insert_pdf || $insert_shop || $insert_anwendung || $insert_mindflash || $insert_kunde || $insert_mitarbeiter || $insert_objekt || $insert_presse || $insert_video || $insert_menu || $insert_vorlage || $insert_warengruppe || $insert_land || $insert_farbe || $insert_icon || $insert_colour || $insert_klasse || $insert_pdf_group || $insert_buchung || $insert_referenz || $insert_pdf_vcard || $insert_leistungen || $insert_event || $insert_team_kontakt ) {
					...
					elseif ($insert_team_kontakt) {
						unset($insert_team_kontakt);
...
';
$get_lang = $morpheus["lan_arr"][$sprache];
include '../page/structure_'.$get_lang.'.inc';
// include '../page/structure_edit_mode_'.$get_lang.'.inc';
