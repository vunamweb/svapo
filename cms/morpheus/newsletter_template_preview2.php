...
<!--<form action="" onsubmit="" name="verwaltung" method="'. ($edit || $neu ? 'post' : 'get').'">
		'. (!$edit && !$neu && !$del ? '<p><select name="nlart" onChange="this.form.submit();"><option value="0"'.($nlart < 1 ? ' selected' : '').'>All brands</option><option value="1"'.($nlart == 1 ? ' selected' : '').'>Jaguar</option><option value="2"'.($nlart == 2 ? ' selected' : '').'>Land Rover</option></select>' : '') .'</form>-->
...
function liste($nlart) {
	...
	while ($row = mysqli_fetch_object($res)) {
		$edit = $row->$id;
		$echo .= '			<tr>
			<td width="150" nowrap valign="top"><p><b>'.$row->$anz.'</b></p></td>
			<td ><img src="../images/'.$row->nltpreview.'" alt="" class="img-responsive" /></td>
		</tr>
		<tr>
			<td colspan="2" style="background:#666; height:3px;"></td>
		</tr>
';
	}
