...
$arr_form = array(
	...
	array("platzhalter", "Platzhalter",'<textarea name="#n#" style="height:120px;#s#">#v#</textarea>'),
	array("platzhaltertext", "Platzhalter Text",'<textarea name="#n#" style="height:120px;#s#">#v#</textarea>'),
...
function liste() {
	...
	$echo .= '<p>&nbsp;</p><table width="100%" cellspacing="0">';
...
#	$sql = "SELECT * FROM $db WHERE 1 ";
	...
	while ($row = mysqli_fetch_object($res)) {	
		...
		$echo .= '<tr>
			<td width="60">'.$y.'</td>
			<td width="550"><a href="?edit='.$edit.'"><strong>'.$row->$anz4.' '.$row->$anz.'</strong> - '.$row->$anz2.', '.$row->$anz3.'</a></td>
...
elseif ($delete) {
	$sql = "DELETE FROM morp_newsletter_vt_csv WHERE vid=$delete";
	$res = safe_query($sql);	
}
