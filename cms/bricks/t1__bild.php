...
if($text) {
	...
	$output .= $extLink.'<img src="'.($type == "svg" ? $img_pfad.$inm : $dir.$imageFolder.urlencode($folder).'/'.urlencode($inm).'?w='.$w).'" alt="'.$altText.'" class="img-fluid '.$ausrichtungArray[$ausrichtung].'" width="'.($img_w ? $img_w : $w).'"'.($img_h ? ' height="'.$img_h.'"' : '').' />'.($extLink ? '</a>' : '');
}
